...
%left '<' '>'
%%
start : cond ';' { yylex.(*lexer).val = $1 }
      | ASSIGN IDENTIFIER '=' filtered ';' {yylex.(*lexer).Assignment = Assignment{$2, &expression{$4}}}
...
expr : LITERAL { val := $1; $$ = func(Context) values.Value { return values.ValueOf(val) } }
...
filter_params : expr { $$ = []valueFn{$1} }
              | filter_params ',' expr  { $$ = append($1, $3) }
			  ;
